...
import com.lia.liaprove.infrastructure.dtos.algorithms.genetic.UpdateRecruiterVoteWeightsRequest;
...
public class AdminGeneticAlgorithmController {
    ...
    public ResponseEntity<Map<UUID, Integer>> adjustAllRecruiterWeights(@Valid @RequestBody AdjustGeneticWeightsRequest request) {
        ...
        Map<UUID, Integer> result = manageVoteWeightUseCase.adjustAllRecruiterWeights(dryRun);
    ...
    public ResponseEntity<Void> setRecruiterVoteWeight(@PathVariable UUID id,
        ...
        manageVoteWeightUseCase.setRecruiterVoteWeight(id, request.weight());
        return ResponseEntity.ok().build();
    }

    @PatchMapping("/recruiters/vote-weights")
    public ResponseEntity<Void> applyRecruiterVoteWeights(@Valid @RequestBody UpdateRecruiterVoteWeightsRequest request) {
        Map<UUID, Integer> weights = new java.util.HashMap<>();
        for (UpdateRecruiterVoteWeightsRequest.RecruiterVoteWeightItem item : request.weights()) {
            weights.put(item.recruiterId(), item.weight());
        }
        manageVoteWeightUseCase.applyManualWeights(weights);
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<Void> setRoleMultiplier(@PathVariable UserRole role,
        ...
        manageVoteWeightUseCase.setRoleMultiplier(role, request.multiplier());
        return ResponseEntity.ok().build();
    }
    ...
    public ResponseEntity<Void> setRecruiterMultiplier(@PathVariable UUID id,
                                                       @Valid @RequestBody UpdateMultiplierRequest request) {
        manageVoteWeightUseCase.setRecruiterMultiplier(id, request.multiplier());
        return ResponseEntity.ok().build();
    }
